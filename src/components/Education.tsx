    ...
    <section id="education" className="py-20 bg-gray-800">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative">
          {/* Timeline line */}
          <div className="absolute left-1/2 transform -translate-x-px h-full w-0.5 bg-teal-500/50 hidden md:block"></div>
          
          <div className="space-y-12">
            {educationData.map((edu, index) => (
              <div key={index} className={`relative flex items-center ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
                {/* Timeline dot */}
                <div className="absolute left-1/2 transform -translate-x-1/2 w-4 h-4 bg-teal-500 rounded-full border-4 border-gray-800 shadow-lg hidden md:block z-10"></div>
                
                <div className={`w-full md:w-5/12 ${index % 2 === 0 ? 'md:pr-8' : 'md:pl-8'}`}>
                  <Card className={`border-gray-600/50 shadow-lg hover:shadow-xl transition-shadow duration-300 border-l-4 ${getColorClasses(edu.color)} backdrop-blur-sm`}>
                    <CardHeader>
                      <CardTitle className="text-xl font-semibold text-white">
                        {edu.degree}
                      </CardTitle>
                      <div className="text-teal-400 font-medium">{edu.institution}</div>
